    ...
    def _setup_drag_drop(self):
        ...
        try:
            ...
            try:
                ...
                # Создаем обертку для drag-and-drop на корневом окне
                dnd_root = TkinterDnD.DnDWrapper(tk_root)
                dnd_root.drop_target_register(DND_FILES)
                dnd_root.dnd_bind('<<Drop>>', self._on_drop)
                
                # Также регистрируем на основном контейнере для лучшей поддержки
                try:
                    # Получаем основной контейнер (scrollable frame или main frame)
                    main_container = None
                    ...
                    if main_container:
                        container_tk = main_container.tk if hasattr(main_container, 'tk') else main_container
                        if hasattr(container_tk, 'drop_target_register'):
                            container_tk.drop_target_register(DND_FILES)
                            container_tk.dnd_bind('<<Drop>>', self._on_drop)
                except Exception as e:
                    logger.debug(f"Не удалось зарегистрировать drag-and-drop на контейнере: {e}")
