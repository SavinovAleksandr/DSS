# Проблема с устойчивостью при проверке параметров узла КЗ
...
1. **Проверить, где именно происходит КЗ:**
   - Если `node_kz == ip` и `distance ≈ 0`, то КЗ в узле начала
   - Если `node_kz == ip` и `distance > 0`, то КЗ на линии на расстоянии `distance` от узла начала

2. **Проверить параметры для узла начала:**
   - Если КЗ в узле начала (distance ≈ 0), то параметры должны быть почти нулевыми
   - Если КЗ на линии (distance > 0), то параметры `begin_r/begin_x` соответствуют части линии от узла начала до точки КЗ

3. **Проверить соответствие параметров:**
   - `begin_r + end_r = 13.57 Ом` ✓
   - `begin_x + end_x = 53.903 Ом` ✓
   - Это означает, что параметры распределены правильно

4. **Возможная проблема:**
   - Если при проверке устойчивости в узле 40903102 с параметрами `r=2.013210, x=7.996910` устойчивость нарушается, это может означать, что эти параметры не соответствуют фактическому состоянию системы
   - Возможно, нужно использовать параметры всей линии или другие параметры
...
2. **Какие параметры нужно использовать для проверки?**
   - Параметры части линии от узла начала до точки КЗ (`begin_r/begin_x`)?
   - Параметры всей линии?
   - Другие параметры?

3. **Что означает "устойчивость нарушается"?**
